...
def objective(args: argparse.Namespace, trial: Trial) -> float:
    ...
    args.weight_kl_z = trial.suggest_float("weight_kl_z", 1e-9, 5e-6, log=True)
    args.weight_x_pred = trial.suggest_float("weight_x_pred", 1e-4, 5e-2, log=True)
    args.weight_z_pred = trial.suggest_float("weight_z_pred", 1e-4, 5e-2, log=True)
    args.weight_spectral = trial.suggest_float("weight_spectral", 1e-4, 5e-2, log=True)
